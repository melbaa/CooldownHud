...
--   Updated toc to 4211
-- 2005.02.13
--   When moving s from one button to another without doing the un3 inbetween, 
...
local function CooldownHud_SetLocks()
	...
	for i = 1, 12, 1 do
		...
		if (CooldownHud_Config[CooldownHud_Player].locklist[i]) then
			local spellNum = CooldownHud_FindSpell (CooldownHud_Config[CooldownHud_Player].locklist[i], true);
			if (spellNum) then
	...
	elseif (firstword == "lock") then
		...
		if (buttonNum and buttonNum > 0 and buttonNum <= 12) then
			...
			if (spellNum) then
				...
				DEFAULT_CHAT_FRAME:AddMessage("Cooldown Hud: Button "..buttonNum.." locked to "..spellName.." id:"..spellNum);
				CooldownHud_SetLocks();
			else
			...
			for i = 1, 12, 1 do
				if (CooldownHud_Config[CooldownHud_Player].locklist[i]) then
					...
					local b_spell = CooldownHud_FindSpell (CooldownHud_Config[CooldownHud_Player].locklist[i], true);
